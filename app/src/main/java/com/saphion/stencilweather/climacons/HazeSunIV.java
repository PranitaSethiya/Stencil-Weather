...
public class HazeSunIV extends ImageView {
	...
	public HazeSunIV(Context context) {
		super(context);
	}

	public HazeSunIV(Context context, int height, int width, int tint) {
		super(context);
		this.height = height;
		this.width = width;
...
//		options.inScaled = false;
		...
		sun = Bitmap.createScaledBitmap(sun, (int) (width * 0.7),
				(int) (height * 0.7), true);
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.save();
		canvas.rotate(angle,
				width - (float) (cloud.getWidth()) + cloud.getWidth() / 2,
				(float) cloud.getHeight() / 2);
